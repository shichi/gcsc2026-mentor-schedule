# GCSC2026 Mentor Booking System

## Overview

This directory contains mentor booking data for GCSC2026.

## Files

- `mentor_bookings.json` - Booking data (JSON format)
- `mentor_bookings.md` - Booking status visualization (Markdown format, easy to view on GitHub)

## Usage

### Booking via Discord

Use the following commands in team channels (`#team-01` to `#team-20`):

```
/mentor book session:session_1 mentor:Masatoshi KANEKO
/mentor cancel session:session_1
/mentor my
/mentor status session:session_1
```

### Check Booking Status

Open `mentor_bookings.md` on GitHub to view the overall booking status.

## Session Information

### Session 1: 10:00-12:00
**Mentors:**
- Masatoshi KANEKO (金子 匡俊)
- Julian Brody
- Heedong YOO
- Hyun Joo CHUNG

### Session 2: 13:00-15:00
**Mentors:**
- Kenta MURAOKA (村岡 健太)
- Keiichiro KATAOKA (片岡 慶一郎)
- Seung Hwan MOK
- Hyun Joo CHUNG

### Session 3: 15:30-17:30
**Mentors:**
- Kenta MURAOKA (村岡 健太)
- Takatsugu KONNO (紺野 貴嗣)
- Seung Hwan MOK
- Hyun Joo CHUNG

### Faculty Night: 20:00-22:00
**Mentors:**
- Heedong YOO
- Seung Hwan MOK
- Hyun Joo CHUNG
- Hitotsubashi University Faculty (一橋大学教員)

※ Bookings available (priority for teams with slower progress)
